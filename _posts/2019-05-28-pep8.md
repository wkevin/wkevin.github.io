...
你还可以创建一个 VSCode 的 watching task，或使用 "run on save" 之类的 VSCode扩展，指定保存文件或其他动作时自动执行美化 task。
...
Watcher 的配置中有一个Auto-Edit的选项请不要勾选，否则会在你coding的同时帮你美化，这种感觉我试了一下，非常酸爽，如图：
